...
class TestEntityTypeDetection:
    ...
    def test_switch_entity(self, mock_hass):
        ctrl = HotWaterController(mock_hass, entity_id="switch.boiler_relay")
        assert ctrl.entity_domain == "switch"

    def test_water_heater_entity(self, mock_hass):
        ctrl = HotWaterController(mock_hass, entity_id="water_heater.viessmann_dhw")
        assert ctrl.entity_domain == "water_heater"

    def test_climate_entity(self, mock_hass):
        ctrl = HotWaterController(mock_hass, entity_id="climate.knx_hot_water")
        assert ctrl.entity_domain == "climate"

    def test_no_entity(self, mock_hass):
        ctrl = HotWaterController(mock_hass, entity_id=None)
...
class TestTemperatureReading:
    ...
    def test_switch_reads_separate_sensor(self, mock_hass):
        mock_hass.states.get = lambda eid: _make_state(52.3) if eid == "sensor.water_temp" else None
    ...
    def test_water_heater_reads_current_temperature_attr(self, mock_hass):
        mock_hass.states.get = lambda eid: _make_state("heating",
    ...
    def test_climate_reads_current_temperature_attr(self, mock_hass):
        mock_hass.states.get = lambda eid: _make_state("heat",
            {"current_temperature": 45.0}) if eid == "climate.knx_dhw" else None
        ctrl = HotWaterController(mock_hass, entity_id="climate.knx_dhw")
        assert ctrl.get_current_temperature() == 45.0

    def test_no_temp_sensor_returns_none(self, mock_hass):
        mock_hass.states.get = lambda eid: None
        ctrl = HotWaterController(mock_hass, entity_id="switch.boiler")
        assert ctrl.get_current_temperature() is None

    def test_unavailable_sensor_returns_none(self, mock_hass):
        mock_hass.states.get = lambda eid: _make_state("unavailable")
        ctrl = HotWaterController(mock_hass, entity_id="switch.boiler",
                                   temperature_entity_id="sensor.water_temp")
        assert ctrl.get_current_temperature() is None

    def test_fallback_to_separate_sensor_when_attr_missing(self, mock_hass):
...
class TestTemperatureSafety:
    ...
    def test_safe_when_below_solar_target(self, mock_hass):
        mock_hass.states.get = lambda eid: _make_state(45.0)
        ctrl = HotWaterController(mock_hass, entity_id="switch.boiler",
                                   temperature_entity_id="sensor.temp",
                                   solar_target_temp=60.0)
        assert ctrl.is_temperature_safe() is True

    def test_unsafe_when_above_solar_target(self, mock_hass):
        mock_hass.states.get = lambda eid: _make_state(62.0)
        ctrl = HotWaterController(mock_hass, entity_id="switch.boiler",
                                   temperature_entity_id="sensor.temp",
                                   solar_target_temp=60.0)
        assert ctrl.is_temperature_safe() is False

    def test_safe_when_no_sensor(self, mock_hass):
        mock_hass.states.get = lambda eid: None
        ctrl = HotWaterController(mock_hass, entity_id="switch.boiler")
    ...
    def test_high_solar_target_allows_high_temp(self, mock_hass):
        """Solar target 80°C allows heating above 60°C — Legionella met naturally."""
        mock_hass.states.get = lambda eid: _make_state(65.0)
        ctrl = HotWaterController(mock_hass, entity_id="switch.boiler",
                                   temperature_entity_id="sensor.temp",
    ...
    @pytest.mark.asyncio
    async def test_activate_blocked_above_solar_target(self, mock_hass):
        mock_hass.states.get = lambda eid: _make_state(62.0)
        ctrl = HotWaterController(mock_hass, entity_id="switch.boiler",
                                   temperature_entity_id="sensor.temp",
                                   solar_target_temp=60.0)
...
class TestMultiEntityActivation:
    ...
    @pytest.mark.asyncio
    async def test_switch_activation(self, mock_hass):
        mock_hass.states.get = lambda eid: _make_state(40.0)
        ctrl = HotWaterController(mock_hass, entity_id="switch.boiler",
                                   temperature_entity_id="sensor.temp")
        result = await ctrl.activate(3000)
        assert result > 0
    ...
    @pytest.mark.asyncio
    async def test_water_heater_activation(self, mock_hass):
        mock_hass.states.get = lambda eid: _make_state("idle", {"current_temperature": 40.0})
        ctrl = HotWaterController(mock_hass, entity_id="water_heater.dhw",
    ...
    @pytest.mark.asyncio
    async def test_climate_activation(self, mock_hass):
        mock_hass.states.get = lambda eid: _make_state("off", {"current_temperature": 38.0})
        ctrl = HotWaterController(mock_hass, entity_id="climate.knx_dhw",
                                   solar_target_temp=50.0)
        result = await ctrl.activate(3000)
        assert result == ctrl.rated_power
    ...
    @pytest.mark.asyncio
    async def test_water_heater_deactivation(self, mock_hass):
        mock_hass.states.get = lambda eid: _make_state("heating", {"current_temperature": 50.0})
        ctrl = HotWaterController(mock_hass, entity_id="water_heater.dhw")
    ...
    @pytest.mark.asyncio
    async def test_climate_deactivation(self, mock_hass):
        mock_hass.states.get = lambda eid: _make_state("heat", {"current_temperature": 50.0})
        ctrl = HotWaterController(mock_hass, entity_id="climate.knx_dhw")
        ctrl._status.state = MagicMock()
        await ctrl.deactivate()
        calls = [c for c in mock_hass.services.async_call.call_args_list
...
class TestLegionellaPrevention:
    ...
    def test_overdue_when_never_run(self, mock_hass):
        ctrl = HotWaterController(mock_hass, entity_id="switch.boiler")
        assert ctrl.legionella_overdue is True
        assert ctrl.hours_since_legionella == 999.0

    def test_not_overdue_when_recently_run(self, mock_hass):
        ctrl = HotWaterController(mock_hass, entity_id="switch.boiler",
    ...
    def test_overdue_after_interval(self, mock_hass):
        ctrl = HotWaterController(mock_hass, entity_id="switch.boiler",
                                   legionella_interval_hours=72)
        ctrl._last_legionella_time = datetime.now(tz=dt_util.UTC) - timedelta(hours=80)
        assert ctrl.legionella_overdue is True

    def test_hold_duration_by_temperature(self, mock_hass):
        ctrl60 = HotWaterController(mock_hass, legionella_target_temp=60)
        ctrl65 = HotWaterController(mock_hass, legionella_target_temp=65)
        ctrl70 = HotWaterController(mock_hass, legionella_target_temp=70)
        ctrl80 = HotWaterController(mock_hass, legionella_target_temp=80)
    ...
    def test_target_enforces_minimum_60(self, mock_hass):
        """User cannot set Legionella target below 60°C."""
        ctrl = HotWaterController(mock_hass, legionella_target_temp=40)
        assert ctrl.legionella_target_temp == 60.0

    @pytest.mark.asyncio
    async def test_natural_achievement_records_timestamp(self, mock_hass):
    ...
    @pytest.mark.asyncio
    async def test_forced_cycle_starts_when_overdue(self, mock_hass):
        """Force heating when overdue and temp below target."""
        mock_hass.states.get = lambda eid: _make_state(42.0)
        ctrl = HotWaterController(mock_hass, entity_id="switch.boiler",
                                   temperature_entity_id="sensor.temp",
                                   legionella_interval_hours=72)
        # Never run → overdue
    ...
    @pytest.mark.asyncio
    async def test_no_forced_cycle_without_temp_sensor(self, mock_hass):
    ...
    @pytest.mark.asyncio
    async def test_cycle_completes_after_hold(self, mock_hass):
        """Cycle completes after hold duration at target temperature."""
        mock_hass.states.get = lambda eid: _make_state(66.0)
        ctrl = HotWaterController(mock_hass, entity_id="switch.boiler",
                                   temperature_entity_id="sensor.temp",
    ...
    @pytest.mark.asyncio
    async def test_cycle_continues_during_hold(self, mock_hass):
    ...
    @pytest.mark.asyncio
    async def test_cycle_still_heating(self, mock_hass):
        """Cycle returns heating status while below target."""
        mock_hass.states.get = lambda eid: _make_state(50.0)
        ctrl = HotWaterController(mock_hass, entity_id="switch.boiler",
                                   temperature_entity_id="sensor.temp",
                                   legionella_target_temp=65)
        ctrl._legionella_cycle_active = True
    ...
    def test_record_legionella_cycle(self, mock_hass):
...
class TestSerialization:
    ...
    def test_to_dict_includes_legionella_data(self, mock_hass):
        mock_hass.states.get = lambda eid: _make_state(50.0)
        ctrl = HotWaterController(mock_hass, entity_id="switch.boiler",
                                   temperature_entity_id="sensor.temp",
                                   legionella_target_temp=65,
    ...
    def test_to_dict_water_heater(self, mock_hass):
        ctrl = HotWaterController(mock_hass, entity_id="water_heater.dhw")
        d = ctrl.to_dict()
